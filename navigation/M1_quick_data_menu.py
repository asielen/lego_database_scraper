...
def quick_data_menu():
    ...
    def load_set():
        """ Prompts for a set and loads it as the eval set """
        nonlocal eval_set
        set_num = SetInfo.input_set_num()
        eval_set = SetInfo(set_num)

    def set_info():
        """ Prompts for a set number and creates a dump of its basic data """
        nonlocal eval_set
        if eval_set is None: load_set()
        print(eval_set.debug_dump_all())

    def set_report():
        """ Prompts for a set number and creates a csv dump of that set """
        nonlocal eval_set
        if eval_set is None: load_set()
        eval_set.make_set_report()

    def set_links():
        """ Prompts for a set number and returns a list of urls for easy access """
        nonlocal eval_set
        if eval_set is None: load_set()
